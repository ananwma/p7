...
gringo = ["gringo", "-c", "width=7",  "level-core.lp", "level-style.lp", "level-sim.lp", "level-shortcuts.lp"]
reify = ["reify"]
clingo =["clingo", "-", "meta.lp", "metaD.lp", "metaO.lp", "metaS.lp", "--parallel-mode=4", "--outf=2"]
        
GRINGO = subprocess.Popen(gringo, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
REIFY = subprocess.Popen(reify,stdin=GRINGO.stdout, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
CLINGO = subprocess.Popen(clingo,stdin=REIFY.stdout, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
Map = parse_json_result_string(out)
    
print p7_visualize.side_by_side(p7_visualize.render_ascii_dungeon(Map), *[p7_visualize.render_ascii_touch(Map,i) for i in range(1,4)])